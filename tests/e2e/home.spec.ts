...
test.describe('main solar app (/)', () => {
  ...
  test('moons-visible preset sets size scale to 0.01 + URL ss=', async ({ page }) => {
    await page.goto('/')
    await page.getByTestId('solar-settings-toggle').click()
    await page.getByTestId('solar-sizescale-moons').click()
    await expect(page.getByTestId('solar-sizescale-value')).toHaveText('0.010')
    await page.waitForFunction(() => window.location.hash.includes('ss=0.010'))
