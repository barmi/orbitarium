...
def generate_fixtures(out_dir: Path, spk_path: Path | None = None) -> Path:
    ...
    if spk_path is None:
        ...
        if not spk_path.exists():
            download_de440_kernel(spk_path)

    sp.furnsh(str(spk_path))
    try:
        # Body NAIF ids covering all 14 native segments + 6 aliased planet bodies.
        bodies = [10, 1, 2, 3, 4, 5, 6, 7, 8, 9, 199, 299, 301, 399, 499, 599, 699, 799, 899, 999]
        # Spread across the 1900-2150 default range plus an out-of-the-ordinary J2000+12h.
        sample_jds = [
            ...
            2455197.5,  # J2000 + 10y
            2469807.5,  # J2000 + 50y
            2461165.5008007577,  # 2026-05-05 (Work 02 current date)
            2506717.5,  # 2150-12-31
        ]
        fixtures: list[dict[str, object]] = []
        for body in bodies:
            spice_body = str(PLANET_BODY_ALIASES.get(body, body))
            for jd in sample_jds:
                et = (jd - 2451545.0) * 86400.0
                state, _lt = sp.spkezr(spice_body, et, "J2000", "NONE", "0")
                px, py, pz, vx, vy, vz = (float(x) for x in state)
                fixtures.append(
                    {
                        "naif_id": body,
                        "jd_tdb": jd,
                        "position_m": [px * 1000.0, py * 1000.0, pz * 1000.0],
                        "velocity_m_s": [vx * 1000.0, vy * 1000.0, vz * 1000.0],
                    }
                )
    finally:
        sp.kclear()
    ...
    output: dict[str, object] = {
        "_comment": (
            "Generated by orbitarium_tools.de440.generate_fixtures. "
    ...
    out_path = out_dir / "de440-states.json"
